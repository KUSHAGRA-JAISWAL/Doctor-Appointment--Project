...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql = " SELECT * FROM `admin` WHERE `admin_email`='$login_email'";
  $result = mysqli_query($conn, $sql);
  $num = mysqli_num_rows($result);
  if ($num == 1) {
    while ($row = mysqli_fetch_assoc($result)) {
      if (password_verify($login_password, $row['admin_pass'])) {
        $login = true;
        session_start();
...
<div class="main-box container d-flex my-3">
    <div class="container d-flex w-50">
      <img src="/appointmentApp/images/admin.png" alt="patient banner">
    </div>
    <div class="content-box container mt-5 text-center">
      <h1 class="text-center mt-5">ADMIN LOGIN</h1>
      <form action="/appointmentApp/admin_login.php" method="post">
