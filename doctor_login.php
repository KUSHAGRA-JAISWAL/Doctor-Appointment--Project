...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql = " SELECT * FROM `doctor` WHERE `doctor_email`='$login_email'";
  $result = mysqli_query($conn, $sql);
  $num = mysqli_num_rows($result);
  if ($num == 1) {
    while ($row = mysqli_fetch_assoc($result)) {
      $id = $row['doctor_id'];
      if (password_verify($login_password, $row['doctor_pass'])) {
        $login = true;
        session_start();
        $_SESSION['loggedin'] = true;
        $_SESSION['useremail'] = $login_email;
        header("Location: doctor_schedule.php?doctor_id=$id");
      } else {
...
<head>
  ...
  <title><?php echo "Doctor-login"; ?></title>
</head>
...
<div class="main-box container d-flex my-3">
    <div class="content-box container mt-5 text-center ms-5">
      ...
      <form action="/appointmentApp/doctor_login.php" method="post">
    ...
    <div class="container d-flex">
      <img src="/appointmentApp/images/doctorlogin.jpg" alt="doctor banner">
    </div>
  </div>
